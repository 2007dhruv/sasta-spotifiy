...
const UploadSongModal = ({ isOpen, onClose }) => {
  const { uploadSong, loading, error, uploadProgress } = useMusicStore();
  ...
  // Automatically set Artist Name to user's name when modal opens
  useEffect(() => {
    if (isOpen && user && !formData.artist) {
      setFormData(prev => ({ ...prev, artist: user.name }));
    }
  }, [isOpen, user]);
        ...
        >
          {success ? (
            ...
            <>
              ...
              <h2 style={{ fontSize: '28px', color: 'white', marginBottom: '8px' }}>Upload New Song</h2>
              <p style={{ color: 'var(--text-muted)', marginBottom: '32px' }}>Share your music with the world.</p>
              ...
              <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
                ...
                <button 
                  type="submit" 
                  disabled={loading || !files.audio || !files.cover}
                  style={{ 
                    marginTop: '10px', padding: '14px', borderRadius: '12px', border: 'none', color: 'white', fontWeight: '700',
                    background: 'linear-gradient(135deg, var(--accent-primary), var(--accent-secondary))',
                    cursor: (loading || !files.audio || !files.cover) ? 'not-allowed' : 'pointer',
